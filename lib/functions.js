...
function lditem2obj(lditem){
  ...
  if (lditem.mail){ o.mail=lditem.mail; }
  ...
  if ((lditem.cn!=undefined)||(lditem.ou!=undefined)) {
    if (lditem.objectClass.indexOf('organizationalUnit')>-1){
      ...
      o.type='USER';
      o.name=lditem.cn;
      o.principalname=lditem.userPrincipalName;
      if (o.name=='undefined'){
        o.name=lditem.sAMAccountName;
      }
      var phones=[];
      if (lditem.telephoneNumber){
        phones.push(lditem.telephoneNumber);
      }
      if (lditem.otherTelephone){
        if( Object.prototype.toString.call( lditem.otherTelephone ) === '[object Array]' ) {
          phones.concat(lditem.otherTelephone);
        } else {
          phones.push(lditem.otherTelephone);
        }
      }
      o.phone=[];
      if (phones.length>0){
...
exports.searchInSGroups=function(options,keyword,cb){
  ...
  var opts = {
    scope: 'sub',
    filter: '(&(objectClass=group)(|(name=*'+keyword+'*)(mail=*'+keyword+'*))(groupType:1.2.840.113556.1.4.803:=2147483648))'
  };
...
exports.getUsersInGroups=function(options,keyword,cb){
  ...
  var opts = {
    scope: 'sub',
    filter : '(&(objectCategory=person)(objectCategory=user)(memberOf='+keyword+')(userAccountControl:1.2.840.113556.1.4.803:=512)(!(isDeleted=*)))'
  };
  client.bind(options.adminuser,options.adminpass,function(e){
    if (e) {
      console.log("search:ClientBind:"+e.message);
  ...
  client.bind(options.adminuser,options.adminpass,function(e){
    if (e) {
      ...
      client.search(options.base,opts,function(err,res) {
        ...
        res.on('end', function(result) {
          if (items.length>0) {
            var userclient=ldap.createClient({url:'ldaps://'+options.host+':636'});
